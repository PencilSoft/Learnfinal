...
public class Learn {
    ...
    Path file=Paths.get("./learn_and_teach.in");
try (BufferedReader reader = Files.newBufferedReader(file, charset)) {
    String line = null;
   char   data [][]=null;
      line = reader.readLine();
     data = new char [158][800] ;
     
     totalCommend=new String [158*800];
  ...
  System.out.println("-------------------------------------------------");
   ...
   for (int i = 0;i<158;i++){
    for (int j=0;j<800;j++){
        ...
        if (i-dim>=0&i+dim<158&j-dim>=0&j+dim<800){
            t=0;
